...
mod tests {
    ...
    #[test]
    fn render_input_field_shows_label_and_value() {
        let theme = Theme::default();
        let state = FieldState::default();
        let line = FormFieldRenderer::render_input_field("Amount", "100.00", &state, &theme);
    ...
    #[test]
    fn render_input_field_shows_required_indicator() {
        let theme = Theme::default();
        let state = FieldState {
            required: true,
            ..Default::default()
        };
        let line = FormFieldRenderer::render_input_field("Amount", "", &state, &theme);
        let text: String = line.spans.iter().map(|s| s.content.as_ref()).collect();
        assert!(text.contains("*"));
    }

    #[test]
    fn render_input_field_shows_error_when_touched() {
        let theme = Theme::default();
        let state = FieldState {
            touched: true,
        ...
        let line = FormFieldRenderer::render_input_field("Amount", "abc", &state, &theme);
        let text: String = line.spans.iter().map(|s| s.content.as_ref()).collect();
